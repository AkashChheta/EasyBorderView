...
public class BorderEditText extends android.support.v7.widget.AppCompatEditText {
    ...
    private void createIntit(Context context, AttributeSet attrs) {
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.BorderEditText, 0, 0);
            ...
            mfontType = a.getString(R.styleable.BorderEditText_fonttypeEdit);
    ...
    public void setUnderLine(boolean isunderlinewshow) {
        this.misUnderLline = isunderlinewshow;
        try {
            if (isunderlinewshow == true) {
                this.setPaintFlags(this.getPaintFlags() | Paint.UNDERLINE_TEXT_FLAG);
            }
